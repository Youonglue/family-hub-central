      ...
      {MONTHS.map((name, mi) => {
        ...
        return (
          <button 
            ...
            className="rounded-3xl sm:rounded-[2.5rem] border-2 sm:border-4 border-slate-50 bg-white p-4 sm:p-6 text-left shadow-md hover:shadow-xl transition-all group cursor-pointer active:scale-98"
          >
            <p className="mb-3 font-black text-lg sm:text-xl uppercase italic tracking-tight group-hover:text-indigo-600 transition-colors">
              {name}
            </p>
            
            <div className="grid grid-cols-7 gap-1 text-[8px] sm:text-[9px] font-black text-slate-400">
              {days.map(d => {
                const key = ymd(d);
                ...
                return (
                  <div 
                    ...
                    className={`aspect-square grid place-items-center relative ${
                      d.getMonth() === mi ? "text-slate-900" : "text-slate-200"
                    }`}
                  >
                    {d.getDate()}
                    {hasEv && d.getMonth() === mi && (
                      <div className="absolute bottom-0.5 size-1.5 bg-indigo-600 rounded-full" />
                    )}
                  </div>
                );
              })}
