...
export interface AvatarConfig {
  bg: "amber" | "pink" | "emerald" | "sky" | "rose" | "violet";
  face: "happy" | "determined" | "cool" | "starEyes";
  hair: "none" | "spiky" | "wizard" | "braids" | "curls" | "cap";
  accessory: "none" | "crown" | "glasses" | "headphones" | "sword";
}
...
export function Avatar({ config, className = "size-20" }: { config: AvatarConfig; className?: string }) {
  const bgStyles: Record<AvatarConfig["bg"], string> = {
    amber: "var(--kid-amber)",
    pink: "var(--kid-pink)",
    emerald: "var(--kid-emerald)",
    sky: "var(--kid-sky)",
    rose: "var(--kid-rose)",
    violet: "var(--kid-violet)",
  };

  const selectedBg = bgStyles[config.bg] || "var(--kid-amber)";

  return (
    <div 
      className={`rounded-[2rem] overflow-hidden shadow-inner flex items-center justify-center shrink-0 ${className}`}
      style={{ backgroundColor: selectedBg }}
    >
      <svg viewBox="0 0 100 100" className="w-full h-full select-none pointer-events-none">
        
        {/* Layer 1: Body Base & Shading */}
        <path d="M22 85 C22 68, 30 60, 50 60 C70 60, 78 68, 78 85 Z" fill="#cbd5e1" />
        <path d="M35 85 C35 72, 40 68, 50 68 C60 68, 65 72, 65 85 Z" fill="#f1f5f9" /> {/* Inner shirt */}
        <circle cx="50" cy="55" r="5" fill="#fbcfe8" /> {/* Neck */}
        <circle cx="50" cy="42" r="19" fill="#fdedd3" /> {/* Detailed Skin Face */}

        {/* Layer 2: Eyes / Expressions (Detailed & Stylized) */}
        {config.face === "happy" && (
          <g>
            {/* Left Eye */}
        ...
        {config.face === "cool" && (
          <g stroke="#1e293b" strokeWidth="3" fill="#1e293b">
            {/* Sleek Sunglasses with reflections */}
        ...
        {config.hair === "cap" && (
          <g>
            <path d="M30 35 Q50 18 70 35 Z" fill="#ef4444" /> {/* Dome */}
            <path d="M45 32 L78 32 Q82 35 77 39 L45 36 Z" fill="#dc2626" /> {/* Brim */}
            <circle cx="50" cy="23" r="2.5" fill="#ca8a04" /> {/* Top button */}
          </g>
        )}

        {/* Layer 4: Accessories (Illustrative Overlays) */}
